    ...
    .then(function(response){
        ...
        if(data != "Account Doesn't exist!"){
			$scope.showConfirm = function(){
				$ionicLoading.show({
					template: '<center><img src="img/loading.gif" width=100% height=100%><br>Logging in to your account..</center>',
					duration: 3000
				});
...
.controller("FirstCtrl",['$scope','$http', function($scope,$http,$ionicModal,$ionicLoading){
  ...
  $http.post('http://localhost/php/medicineDetails.php').success(function(data){
...
.controller("SecondCtrl",['$scope','$http', function($scope,$http,$ionicModal){
  ...
  getInfo();
  function getInfo(){
  $http.post('http://localhost/php/announcementDetails.php').success(function(data){
  // Stored the returned data into scoper
  $scope.details = data;
  });
  }
...
.controller('name', function($scope, $http,$state) {
	...
	getInfos();
	function getInfos(){
		$http.post('http://localhost/php/account_pull.php',{"accn":getInfos.acc = localStorage.getItem("name")}).success(function(data){
...
.controller('AccountCtrl', function($scope, $http,$state,$ionicLoading) {
	setInterval(function(){

	var acc = localStorage.getItem("name");

  $ionicLoading.show({
    template: '<center><img src="img/loading.gif" width=70% height=70%><br>Preparing your records ..</center>',
    duration: 3000
  });
